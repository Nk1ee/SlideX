...
export function scoreImageRelevance(candidate: ImageCandidate, query: ImageSearchQuery): ImageRelevanceReport {
  const searchable = `${candidate.altText} ${candidate.title ?? ''}`.trim();
  if (!searchable) return { accepted: false, score: 0, matchedTerms: [], reason: 'Image has no searchable description' };
  const generic = GENERIC_PATTERNS.find((pattern) => pattern.test(searchable));
  if (generic) return { accepted: false, score: 0, matchedTerms: [], reason: `Generic stock pattern rejected: ${generic.source}` };
  ...
  if (score < 0.25) return { accepted: false, score, matchedTerms, reason: 'Candidate does not provide enough semantic term overlap' };
