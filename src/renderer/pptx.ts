...
/** Render only layouts registered in this extraction: title and sources. */
export async function renderPresentation(presentation: Presentation): Promise<Uint8Array> {
  const unsupported = presentation.slides.find((slide) => slide.layout !== 'title' && slide.layout !== 'sources');
  ...
  pptx.company = 'SlideX';
  presentation.slides.forEach((slide) => slide.layout === 'title' ? renderTitleSlide(presentation, slide, pptx) : renderSourcesSlide(slide, pptx));
