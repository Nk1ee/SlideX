...
export const IMPLEMENTED_LAYOUTS: ReadonlySet<Layout> = new Set([
  'title',
  'hero',
  'image_text',
  'two_column',
...
export type ValTownRendererOptions = {
...
function configuredValue(readEnvironment: EnvironmentReader, name: string): string {
...
function needsRenderedImage(presentation: Presentation): boolean {
...
function healthResponse(readEnvironment: EnvironmentReader): Response {
...
export function createValTownRendererHandler(options: ValTownRendererOptions): (request: Request) => Promise<Response> {
  const fetchImpl = options.fetchImpl ?? fetch;

  return async (request: Request): Promise<Response> => {
    if (request.method === 'GET') return healthResponse(options.readEnvironment);
    ...
    if (!renderToken) return errorResponse('SERVER_NOT_CONFIGURED', 'SLIDEX_RENDER_TOKEN is not configured', 503);
    if (!isAuthorized(request, renderToken)) return errorResponse('UNAUTHORIZED', 'Valid bearer token required', 401);
    ...
    if (!wireResult.success) {
      return errorResponse(
        ...
        'Request does not match the SlideX render contract',
        422,
        wireResult.error.issues.map((issue) => ({
