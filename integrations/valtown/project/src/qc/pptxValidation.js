...
export async function validatePptxBinary(binary, options = {}) {
    ...
    if (!entries.some((entry) => entry === 'ppt/'))
        issues.push('Missing ppt/ directory');
    if (!entries.some((entry) => entry === 'ppt/slides/'))
        issues.push('Missing ppt/slides/ directory');
    ...
    if (options.expectedSlideCount !== undefined && slideEntries.length !== options.expectedSlideCount)
        issues.push(`Expected ${options.expectedSlideCount} slides, found ${slideEntries.length}`);
    for (const entry of slideEntries) {
        const xml = await zip.file(entry).async('text');
        if (!xml.includes('<p:sld') || !xml.includes('<p:spTree'))
            issues.push(`${entry} is missing required slide XML`);
        if (!xml.includes('<a:t>') && !xml.includes('<p:pic'))
            issues.push(`${entry} appears empty`);
    }
    const mediaEntries = entries.filter((entry) => entry.startsWith('ppt/media/'));
    if (options.imagesExpected === true && mediaEntries.length === 0)
        issues.push('Images were expected but ppt/media/ is empty');
    return { ok: issues.length === 0, slideCount: slideEntries.length, entries, issues };
}
export async function assertValidPptx(binary, options = {}) {
