...
import { imageAiQualityReportSchema } from '../qc/visualAi.js';
        ...
        for (const provider of options.providers) {
            ...
            try {
                for (const candidate of await searchVisual(provider, slide.visual)) {
                    ...
                    if (report.accepted)
                        scored.push({ candidate, score: report.score });
                    else
                        options.onIssue?.({ provider: provider.id, reason: report.reason ?? 'Relevance check failed' });
                }
            }
            catch {
