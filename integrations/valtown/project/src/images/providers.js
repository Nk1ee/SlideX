    ...
    return {
        ...
        async search(query) {
            ...
            for (const raw of payload.results) {
                ...
                const urls = object(item.urls);
                const user = object(item.user);
                const id = string(item.id);
                const sourceUrl = httpsUrl(links?.html, ['unsplash.com']);
                ...
                const author = string(user?.name);
                const authorUrl = httpsUrl(object(user?.links)?.html, ['unsplash.com']);
                ...
                if (!id || !sourceUrl || !rawImageUrl || !downloadLocation || !author || !authorUrl || !width || !height || !altText)
                    continue;
...
export function createWikimediaProvider(options = {}) {
    const fetchImpl = options.fetchImpl ?? fetch;
    ...
    return {
        ...
        async search(query) {
            ...
            for (const [key, value] of Object.entries({ action: 'query', generator: 'search', gsrsearch: query.queryEn, gsrnamespace: '6', gsrlimit: '30', prop: 'imageinfo', iiprop: 'url|mime|size|extmetadata', iiurlwidth: '1600', format: 'json', formatversion: '2' }))
            ...
            for (const raw of pages) {
                ...
                if (!page || !info)
                    continue;
                const id = positiveInt(page.pageid);
                ...
                if (!isPublicDomain && !isAttribution)
                    continue;
                if (isAttribution && (!author || !licenseUrl))
                    continue;
                ...
                if (!id || !title || !imageUrl || !sourceUrl || !width || !height || !mimeType || !['image/jpeg', 'image/png'].includes(mimeType))
                    continue;
