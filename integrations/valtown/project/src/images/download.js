...
function trustedUrl(raw, hosts) {
    ...
    if (url.protocol !== 'https:' || !hosts.includes(url.hostname.toLowerCase()))
        throw new Error(`Image URL must use a trusted host: ${hosts.join(' or ')}`);
    return url;
}
function dimensions(bytes, mimeType) {
    ...
    if (mimeType === 'image/png') {
        ...
        if (bytes.length < 33 || !signature.every((value, index) => bytes[index] === value) || String.fromCharCode(...bytes.subarray(12, 16)) !== 'IHDR')
...
export async function downloadImage(candidate, options = {}) {
    ...
    if (!hosts)
        throw new Error(`Unsupported image provider: ${candidate.provider}`);
    const url = trustedUrl(candidate.imageUrl, hosts);
    if (!candidate.providerId || !candidate.license || !candidate.author)
        throw new Error('Image provenance is incomplete');
    trustedUrl(candidate.sourceUrl, [candidate.provider === 'wikimedia' ? 'commons.wikimedia.org' : 'unsplash.com']);
    if (candidate.provider === 'unsplash' && (!candidate.authorUrl || !candidate.downloadLocation))
        throw new Error('Unsplash attribution or tracking URL is missing');
    if (candidate.provider === 'wikimedia' && /^CC BY\b/i.test(candidate.license) && !candidate.licenseUrl)
        throw new Error('Wikimedia attribution license URL is missing');
    const response = await fetchImpl(url, { redirect: 'error', signal: AbortSignal.timeout(10000) });
    if (!response.ok)
        throw new Error(`Image download failed with HTTP ${response.status}`);
    const contentType = response.headers.get('content-type')?.split(';')[0]?.trim().toLowerCase();
    if (contentType !== candidate.mimeType || !['image/jpeg', 'image/png'].includes(candidate.mimeType))
        throw new Error('Image response MIME does not match candidate');
    const bytes = await boundedBytes(response);
    const actual = dimensions(bytes, candidate.mimeType);
    if (actual.width < (options.minWidth ?? 400) || actual.height < (options.minHeight ?? 225))
        throw new Error('Image resolution is too small for PPTX');
    if (candidate.provider === 'unsplash') {
        const key = options.unsplashAccessKey ?? process.env.UNSPLASH_ACCESS_KEY ?? '';
        if (!key.trim())
            throw new Error('Unsplash download tracking needs UNSPLASH_ACCESS_KEY');
        ...
        if (!tracking.ok)
            throw new Error(`Unsplash download tracking failed with HTTP ${tracking.status}`);
    }
