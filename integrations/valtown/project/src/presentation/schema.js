...
export const chartSchema = z.strictObject({
    ...
    if (chart.kind === 'pie' || chart.kind === 'doughnut') {
        if (chart.series.length !== 1)
            ctx.addIssue({ code: 'custom', path: ['series'], message: 'Pie and doughnut charts require exactly one series' });
        const values = chart.series[0]?.values ?? [];
        if (values.some((value) => value < 0))
            ctx.addIssue({ code: 'custom', path: ['series', 0, 'values'], message: 'Pie and doughnut charts cannot contain negative values' });
        if (values.length > 0 && values.every((value) => value === 0))
            ctx.addIssue({ code: 'custom', path: ['series', 0, 'values'], message: 'Pie and doughnut charts require at least one positive value' });
    }
});
export const slideSchema = z.strictObject({
    ...
    const require = (condition, field, message) => {
        if (!condition)
    ...
    switch (slide.layout) {
        case 'title': break;
        case 'hero':
            require(Boolean(slide.subtitle.trim()), 'subtitle', 'Hero needs a supplied statement');
            break;
        case 'image_text':
            require(slide.bullets.length > 0, 'bullets', 'Image/text needs text');
            break;
        case 'two_column':
            require(slide.columns.length === 2, 'columns', 'Exactly two columns required');
            break;
        case 'three_cards':
            require(slide.cards.length === 3, 'cards', 'Exactly three cards required');
            break;
        case 'comparison':
            require(slide.comparison !== null, 'comparison', 'Comparison required');
            break;
        case 'timeline':
            require(slide.timeline.length > 0, 'timeline', 'Timeline required');
            break;
        case 'statistics':
            require(slide.statistics !== null, 'statistics', 'Statistics required');
            slide.statistics?.forEach((statistic, index) => {
                if (!statistic.source)
                    ctx.addIssue({ code: 'custom', path: ['statistics', index, 'source'], message: 'Statistics require provenance; no invented values' });
            });
            break;
        case 'chart':
            require(slide.chart !== null, 'chart', 'Chart data required');
            break;
        case 'process':
            require(slide.steps.length > 0, 'steps', 'Process steps required');
            break;
        case 'definition':
            require(slide.definition !== null, 'definition', 'Definition required');
            break;
        case 'quote':
            require(slide.quote !== null, 'quote', 'Quote required');
            break;
        case 'conclusion':
            require(slide.cards.length === 3, 'cards', 'Three supplied takeaways required');
            break;
        case 'sources':
            require(slide.sources.length > 0, 'sources', 'Sources required');
            break;
    }
...
export const presentationSchema = z.strictObject({
    ...
    data.slides.forEach((slide, index) => {
        if (slide.number !== index + 1)
            ctx.addIssue({ code: 'custom', path: ['slides', index, 'number'], message: 'Slide numbers must be sequential and unique' });
