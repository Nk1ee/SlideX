...
function renderTitleSlide(presentation, slideData, pptx, THEME) {
    ...
    if (titleFit.overflow)
        throw new Error(`Title overflows at minimum ${titleStyle.minFontSize}pt`);
...
function renderHeroSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Hero layout does not support images in the first extraction');
    if (!slideData.subtitle.trim())
        throw new Error('Hero layout requires supplied subtitle thesis');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    ...
    if (titleFit.overflow)
        throw new Error(`Hero title overflows at minimum ${titleStyle.minFontSize}pt`);
    ...
    if (thesisFit.overflow)
        throw new Error(`Hero thesis overflows at minimum ${heroStyle.minFontSize}pt`);
...
function renderDefinitionSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Definition layout cannot contain an image');
    if (slideData.definition === null)
        throw new Error('Definition layout requires supplied definition data');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    const labelStyle = typographyFor('LABEL');
    const titleStyle = typographyFor('TITLE');
    ...
    const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
    if (titleFit.overflow)
        throw new Error(`Definition title overflows at minimum ${titleStyle.minFontSize}pt`);
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
    ...
    if (termFit.overflow || bodyFit.overflow)
        throw new Error('Definition content overflows at readable minimum');
...
function renderQuoteSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Quote layout cannot contain an image in the first extraction');
    if (slideData.quote === null)
        throw new Error('Quote layout requires supplied quote data');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    const labelStyle = typographyFor('LABEL');
    const titleStyle = typographyFor('TITLE');
    ...
    const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
    if (titleFit.overflow)
        throw new Error(`Quote title overflows at minimum ${titleStyle.minFontSize}pt`);
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
    ...
    if (quoteFit.overflow)
        throw new Error(`Quote text overflows at minimum ${quoteStyle.minFontSize}pt`);
    ...
    if (attributionFit.overflow)
        throw new Error(`Quote attribution overflows at minimum ${bodyStyle.minFontSize}pt`);
...
function renderTwoColumnBlock(slide, column, columnIndex, x, width, startY, safeBottomY, THEME) {
    ...
    if (headingFit.overflow)
        throw new Error(`Two-column heading ${columnIndex + 1} overflows at readable minimum`);
    ...
    if (bodyFit.overflow)
        throw new Error(`Two-column content ${columnIndex + 1} overflows at readable minimum`);
    const bodyHeight = Math.max(0.72, bodyFit.estimatedHeight + 0.04);
    const bottomY = bodyY + bodyHeight;
    if (bottomY > safeBottomY)
        throw new Error(`Two-column content ${columnIndex + 1} exceeds the safe slide height`);
...
function renderTwoColumnSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Two-column layout cannot contain an image');
    if (slideData.columns.length !== 2)
        throw new Error('Two-column layout requires exactly two supplied columns');
    for (const [index, column] of slideData.columns.entries()) {
        if (!column.title.trim() || column.items.length === 0)
            throw new Error(`Two-column column ${index + 1} requires a title and supplied items`);
    }
    const titleStyle = typographyFor('TITLE');
    ...
    if (titleFit.overflow)
        throw new Error(`Two-column title overflows at minimum ${titleStyle.minFontSize}pt`);
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    ...
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    const positions = [0.8, 5.25];
    for (const [index, column] of slideData.columns.entries()) {
        renderTwoColumnBlock(slide, column, index, positions[index], 3.75, 1.72, 5.0, THEME);
    }
}
function renderThreeCardBlock(slide, card, cardIndex, x, width, startY, safeBottomY, THEME) {
    ...
    if (titleFit.overflow)
        throw new Error(`Three-cards title ${cardIndex + 1} overflows at readable minimum`);
    ...
    if (bodyFit.overflow)
        throw new Error(`Three-cards text ${cardIndex + 1} overflows at readable minimum`);
    const bodyHeight = Math.max(0.72, bodyFit.estimatedHeight + 0.04);
    const bottomY = bodyY + bodyHeight;
    if (bottomY > safeBottomY)
        throw new Error(`Three-cards content ${cardIndex + 1} exceeds the safe slide height`);
...
function renderThreeCardsSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Three-cards layout cannot contain an image');
    if (slideData.cards.length !== 3)
        throw new Error('Three-cards layout requires exactly three supplied cards');
    for (const [index, card] of slideData.cards.entries()) {
        if (!card.title.trim() || !card.text.trim())
            throw new Error(`Three-cards card ${index + 1} requires supplied title and text`);
    }
    const titleStyle = typographyFor('TITLE');
    const titleFit = fitText({
        text: slideData.title,
    ...
    if (titleFit.overflow)
        throw new Error(`Three-cards slide title overflows at minimum ${titleStyle.minFontSize}pt`);
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
    const positions = [0.8, 3.725, 6.65];
...
function renderComparisonSide(slide, side, sideName, x, width, startY, safeBottomY, THEME) {
    const headingStyle = typographyFor('SUBTITLE', { bold: true });
    const bodyStyle = typographyFor('BODY');
    const headingFit = fitText({
        text: side.title,
    ...
    if (headingFit.overflow)
        throw new Error(`Comparison ${sideName} heading overflows at readable minimum`);
    ...
    if (bodyFit.overflow)
        throw new Error(`Comparison ${sideName} items overflow at readable minimum`);
    const bodyHeight = Math.max(0.72, bodyFit.estimatedHeight + 0.04);
    const bottomY = bodyY + bodyHeight;
    if (bottomY > safeBottomY)
        throw new Error(`Comparison ${sideName} content exceeds the safe slide height`);
...
function renderComparisonSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Comparison layout cannot contain an image');
    if (slideData.comparison === null)
        throw new Error('Comparison layout requires supplied left and right sides');
    for (const [name, side] of [['left', slideData.comparison.left], ['right', slideData.comparison.right]]) {
        if (!side.title.trim() || side.items.length === 0)
            throw new Error(`Comparison ${name} side requires a title and supplied items`);
    }
    const titleStyle = typographyFor('TITLE');
    const titleFit = fitText({
        text: slideData.title,
    ...
    if (titleFit.overflow)
        throw new Error(`Comparison title overflows at minimum ${titleStyle.minFontSize}pt`);
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
...
function renderTimelineEntry(slide, entry, entryIndex, x, width, nodeY, safeBottomY, THEME) {
    ...
    if (dateFit.overflow || titleFit.overflow)
        throw new Error(`Timeline entry ${entryIndex + 1} heading overflows at readable minimum`);
    ...
    if (bodyFit.overflow)
        throw new Error(`Timeline entry ${entryIndex + 1} text overflows at readable minimum`);
    const bodyHeight = Math.max(0.72, bodyFit.estimatedHeight + 0.04);
    const bottomY = bodyY + bodyHeight;
    if (bottomY > safeBottomY)
        throw new Error(`Timeline entry ${entryIndex + 1} exceeds the safe slide height`);
...
function renderTimelineSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Timeline layout cannot contain an image');
    if (slideData.timeline.length === 0)
        throw new Error('Timeline layout requires supplied dated entries');
    if (slideData.timeline.length > 4)
        throw new Error('Timeline layout supports at most four entries at readable size');
    const titleStyle = typographyFor('TITLE');
    const titleFit = fitText({
        text: slideData.title,
        widthInches: 8.4,
    ...
    if (titleFit.overflow)
        throw new Error(`Timeline title overflows at minimum ${titleStyle.minFontSize}pt`);
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
...
function measureProcessStep(step, stepIndex, width, maxHeight) {
    ...
    if (titleFit.overflow)
        throw new Error(`Process step ${stepIndex + 1} title overflows at readable minimum`);
    const titleHeight = Math.max(0.28, titleFit.estimatedHeight);
    const bodyMaxHeight = maxHeight - titleHeight - 0.06;
    if (bodyMaxHeight <= 0)
        throw new Error(`Process step ${stepIndex + 1} has no readable body space`);
    const bodyFit = fitText({
        text: step.text,
        widthInches: width,
    ...
    if (bodyFit.overflow)
        throw new Error(`Process step ${stepIndex + 1} text overflows at readable minimum`);
    const bodyHeight = Math.max(0.3, bodyFit.estimatedHeight + 0.02);
    const height = titleHeight + 0.06 + bodyHeight;
    if (height > maxHeight)
        throw new Error(`Process step ${stepIndex + 1} exceeds its readable height`);
    return { step, titleFit, bodyFit, titleHeight, bodyHeight, height };
}
function renderProcessSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Process layout cannot contain an image');
    if (slideData.steps.length === 0)
        throw new Error('Process layout requires supplied steps');
    if (slideData.steps.length > 4)
        throw new Error('Process layout supports at most four steps at readable size');
    for (const [index, step] of slideData.steps.entries()) {
        if (!step.title.trim() || !step.text.trim())
            throw new Error(`Process step ${index + 1} requires supplied title and text`);
    }
    const titleStyle = typographyFor('TITLE');
    const titleFit = fitText({
        text: slideData.title,
    ...
    if (titleFit.overflow)
        throw new Error(`Process title overflows at minimum ${titleStyle.minFontSize}pt`);
    const startY = 1.62;
    ...
    if (currentY - gap > safeBottomY)
        throw new Error('Process exceeds the safe slide height');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
...
function compactSourceText(source) {
...
function measureStatistic(statistic, statisticIndex, rowHeight) {
    if (!statistic.source)
        throw new Error(`Statistic ${statisticIndex + 1} requires a supplied source`);
    ...
    if (descriptionMaxHeight <= 0)
        throw new Error(`Statistic ${statisticIndex + 1} has no readable description space`);
    ...
    if (valueFit.overflow)
        throw new Error(`Statistic ${statisticIndex + 1} value overflows at readable minimum`);
    if (labelFit.overflow)
        throw new Error(`Statistic ${statisticIndex + 1} label overflows at readable minimum`);
    if (descriptionFit.overflow)
        throw new Error(`Statistic ${statisticIndex + 1} description overflows at readable minimum`);
    if (sourceFit.overflow)
        throw new Error(`Statistic ${statisticIndex + 1} source overflows at readable minimum`);
    return { statistic, valueFit, labelFit, descriptionFit, sourceFit, labelHeight, descriptionHeight, sourceHeight, sourceLabel };
}
function renderStatisticsSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Statistics layout cannot contain an image');
    if (slideData.statistics === null || slideData.statistics.length === 0)
        throw new Error('Statistics layout requires supplied statistics');
    if (slideData.statistics.length > 3)
        throw new Error('Statistics layout supports at most three statistics at readable size');
    const titleStyle = typographyFor('TITLE');
    const titleFit = fitText({
        text: slideData.title,
        widthInches: 8.4,
    ...
    if (titleFit.overflow)
        throw new Error(`Statistics title overflows at minimum ${titleStyle.minFontSize}pt`);
    const startY = 1.55;
...
function renderChartSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Chart layout cannot contain an image');
    if (slideData.chart === null)
        throw new Error('Chart layout requires supplied chart data');
    ...
    const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
    if (titleFit.overflow)
        throw new Error(`Chart title overflows at minimum ${titleStyle.minFontSize}pt`);
    ...
    if (sourceFit.overflow)
        throw new Error('Chart source overflows at readable minimum');
    const circular = chart.kind === 'pie' || chart.kind === 'doughnut';
    const pptxType = chart.kind === 'column' || chart.kind === 'bar' ? 'bar' : chart.kind;
    ...
    const chartOptions = {
        ...
        ...(chart.kind === 'column' ? { barDir: 'col', barGrouping: 'clustered', barGapWidthPct: 65 } : {}),
        ...(chart.kind === 'bar' ? { barDir: 'bar', barGrouping: 'clustered', barGapWidthPct: 55 } : {}),
    ...
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    if (chart.unit.trim())
        slide.addText(`Единица: ${chart.unit}`, { x: 6.2, y: 0.4, w: 3.0, h: 0.25, fontFace: labelStyle.fontFace, fontSize: labelStyle.preferredFontSize, bold: labelStyle.bold, color: THEME.subtitle, align: 'right' });
...
function renderImageTextSlide(slideData, pptx, imageResolver, THEME) {
    ...
    return imageResolver(slideData).then((image) => {
        ...
        if (!image.author?.trim() || !image.license?.trim() || !image.sourceUrl)
            throw new Error('Resolved image has no attribution metadata');
        if (image.provider === 'unsplash' && !image.authorUrl)
            throw new Error('Unsplash photographer profile is missing');
        ...
        if (slideData.visual.type === 'diagram' && fullBleed)
            throw new Error('Diagram requires a contained image placement');
        if (fullBleed) {
        ...
        const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
        if (titleFit.overflow)
            throw new Error(`Image text title overflows at minimum ${titleStyle.minFontSize}pt`);
        slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
        if (!fullBleed)
            slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
        ...
        if (bodyFit.overflow)
            throw new Error(`Image text bullets overflow at minimum ${bodyStyle.minFontSize}pt`);
        ...
        if (creditFit.overflow)
            throw new Error('Image attribution overflows at readable minimum');
...
function sourceText(source) {
    const provenance = [source.author ?? source.organization, source.year === undefined ? undefined : String(source.year)].filter((part) => part !== undefined && part.length > 0).join(', ');
    return `${source.title}${provenance ? ` — ${provenance}` : ''}${source.url ? `\n${source.url}` : ''}`;
}
function renderSourceBlock(slide, source, index, startY, THEME) {
    const bodyStyle = typographyFor('BODY');
    const labelStyle = typographyFor('LABEL');
    const captionStyle = typographyFor('CAPTION');
    const provenance = [source.author ?? source.organization, source.year === undefined ? undefined : String(source.year)].filter((part) => part !== undefined && part.length > 0).join(', ');
    ...
    if (titleFit.overflow || provenanceFit?.overflow || urlFit?.overflow)
        throw new Error(`Source ${index + 1} overflows at readable minimum`);
    let currentY = startY;
...
function renderSourcesSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Sources layout cannot contain an image');
    if (slideData.cards.length > 0)
        throw new Error('Sources layout cannot contain content cards');
    if (slideData.sources.length === 0)
        throw new Error('Sources layout requires supplied sources');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    const labelStyle = typographyFor('LABEL');
    const titleStyle = typographyFor('TITLE');
    ...
    const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
    if (titleFit.overflow)
        throw new Error(`Sources title overflows at minimum ${titleStyle.minFontSize}pt`);
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
    let currentY = 1.65;
    for (const [index, source] of slideData.sources.entries())
        currentY = renderSourceBlock(slide, source, index, currentY, THEME).bottomY + 0.12;
    if (currentY > 5.05)
        throw new Error('Sources exceed the safe slide height');
}
function renderConclusionSlide(slideData, pptx, THEME) {
    if (slideData.visual.needed)
        throw new Error('Conclusion layout cannot contain an image');
    if (slideData.cards.length !== 3)
        throw new Error('Conclusion layout requires exactly three supplied takeaways');
    const slide = pptx.addSlide();
    slide.background = { color: THEME.background };
    const labelStyle = typographyFor('LABEL');
    const titleStyle = typographyFor('TITLE');
    ...
    const titleFit = fitText({ text: slideData.title, widthInches: 8.4, maxHeightInches: 0.6, preferredFontSize: titleStyle.preferredFontSize, minFontSize: titleStyle.minFontSize });
    if (titleFit.overflow)
        throw new Error(`Conclusion title overflows at minimum ${titleStyle.minFontSize}pt`);
    slide.addText(slideData.title, { x: 0.8, y: 0.65, w: 8.4, h: 0.6, fontFace: titleStyle.fontFace, fontSize: titleFit.fontSize, bold: titleStyle.bold, color: THEME.title, valign: 'top', fit: 'shrink' });
    slide.addShape('rect', { x: 0.8, y: 1.35, w: 8.4, h: THEME.dividerHeight, fill: { color: THEME.accent }, line: { color: THEME.accent, transparency: 100 } });
    let currentY = 1.65;
    const bodyStyle = typographyFor('BODY');
    ...
    for (const [index, card] of slideData.cards.entries()) {
        ...
        if (titleFitCard.overflow || bodyFit.overflow)
            throw new Error(`Conclusion takeaway ${index + 1} overflows at readable minimum`);
    ...
    if (currentY > 5.1)
        throw new Error('Conclusion exceeds the safe slide height');
}
/** Render only layouts registered in this extraction. */
export async function renderPresentation(presentation, options = {}) {
    ...
    if (unsupported)
        throw new Error(`Layout not implemented in local renderer: ${unsupported.layout}`);
    const selectedTheme = getPresentationTheme(presentation.presentation.style);
    const THEME = { ...selectedTheme.colors, ...selectedTheme.geometry };
    ...
    for (const slide of presentation.slides) {
        if (slide.layout === 'title')
            renderTitleSlide(presentation, slide, pptx, THEME);
        else if (slide.layout === 'sources')
            renderSourcesSlide(slide, pptx, THEME);
        else if (slide.layout === 'conclusion')
            renderConclusionSlide(slide, pptx, THEME);
        else if (slide.layout === 'definition')
            renderDefinitionSlide(slide, pptx, THEME);
        else if (slide.layout === 'hero')
            renderHeroSlide(slide, pptx, THEME);
        else if (slide.layout === 'quote')
            renderQuoteSlide(slide, pptx, THEME);
        else if (slide.layout === 'two_column')
            renderTwoColumnSlide(slide, pptx, THEME);
        else if (slide.layout === 'three_cards')
            renderThreeCardsSlide(slide, pptx, THEME);
        else if (slide.layout === 'comparison')
            renderComparisonSlide(slide, pptx, THEME);
        else if (slide.layout === 'timeline')
            renderTimelineSlide(slide, pptx, THEME);
        else if (slide.layout === 'statistics')
            renderStatisticsSlide(slide, pptx, THEME);
        else if (slide.layout === 'chart')
            renderChartSlide(slide, pptx, THEME);
        else if (slide.layout === 'process')
            renderProcessSlide(slide, pptx, THEME);
        else {
            if (!options.imageResolver)
                throw new Error('Image text layout requires an imageResolver');
