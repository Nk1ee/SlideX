...
export function typographyFor(role, overrides = {}) {
    return { ...DEFAULT_TYPOGRAPHY[role], ...overrides };
}
export function assertTypographyConfig(config) {
    for (const [role, style] of Object.entries(config)) {
        if (!style.fontFace.trim())
            throw new Error(`${role} fontFace must not be empty`);
        if (style.preferredFontSize <= 0 || style.minFontSize <= 0)
            throw new Error(`${role} font sizes must be positive`);
        if (style.minFontSize > style.preferredFontSize)
            throw new Error(`${role} minFontSize cannot exceed preferredFontSize`);
    }
}
