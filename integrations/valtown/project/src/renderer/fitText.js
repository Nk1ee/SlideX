 ...
 */
export function fitText({ text, widthInches, maxHeightInches, preferredFontSize, minFontSize, lineHeightMultiplier = 1.35, averageCharacterWidthFactor = 0.007, }) {
    if (widthInches <= 0 || maxHeightInches <= 0)
        throw new Error('Text box dimensions must be positive');
    if (preferredFontSize <= 0 || minFontSize <= 0 || minFontSize > preferredFontSize)
        throw new Error('Font sizes must be positive and minFontSize <= preferredFontSize');
    if (lineHeightMultiplier <= 0 || averageCharacterWidthFactor <= 0)
        throw new Error('Typography factors must be positive');
    let fontSize = preferredFontSize;
    let lastResult = { fontSize, estimatedLines: 0, estimatedHeight: 0, overflow: false };
