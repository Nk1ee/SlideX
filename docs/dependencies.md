...
Это зафиксированный риск, а не подтверждение эксплуатации. Локальный image_text теперь принимает только JPEG/PNG после host, MIME, размера и сигнатуры. Это сужает вход, но не доказывает устранение advisory в транзитивной зависимости; production-включение требует отдельного dependency review.
