...
`process` принимает от одного до четырёх шагов `{ title, text }`. Вертикальная направляющая и номера показывают порядок выполнения; дат и временной оси здесь нет. Высота каждого шага измеряется до размещения следующего. Layout не принимает изображения и не создаёт отсутствующие действия. Более длинный процесс должен быть разделён или семантически сжат на AI/orchestration-слое.

## statistics

`statistics` принимает от одного до трёх показателей `{ value, label, description, source }`. `value` выводится дословно, поэтому `3.2x`, `47%` и `14 pt` не меняют смысл или единицу. У каждого показателя обязателен реальный переданный источник; краткая ссылка видна на слайде, а полный URL хранится в заметках PPTX. Layout не принимает изображения, не создаёт значения и не использует legacy-fallback `78%`. Больший набор показателей требует другого плана или нескольких слайдов.
