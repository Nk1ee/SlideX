...
Все 14 layouts ниже описаны контрактом и поддерживаются локальным renderer; перед render проверяется фактический реестр.
...
| chart | chart с categories, series и source | Редактируемая столбчатая, круговая или кольцевая диаграмма |
...
`chart` создаёт нативный объект PowerPoint, который можно открыть и отредактировать в самом PPTX. Поддерживаются `column` (вертикальные столбцы), `bar` (горизонтальные полосы), `pie` (круговая) и `doughnut` (кольцевая). Renderer не вычисляет факты, не меняет значения и не создаёт отсутствующие категории. Источник и единица приходят через контракт. Круговые виды принимают одну серию без отрицательных значений; столбчатые — до трёх серий.
