...
`presentation`: единый контракт, каталог визуальных тем и сверка с независимым FSM request. `renderer`: PptxGenJS, применение выбранной темы, геометрия, typography, fitting и композиция. `images`: поиск по visual concept/query_en, provider metadata, relevance, dedupe и явный selector. `qc`: до- и послерендерные проверки. Image subsystem не генерирует content и отдаёт null, когда релевантного результата нет.
...
Trusted education context вводится миграционно: school хранит дословный `schoolClass`, college/university — дословный `course`, а существующее поле `group` сохраняется. `validatePresentation` запрещает Gemini менять этот объект. Пока legacy n8n не обновлён, поле optional; после переключения нового FSM оно должно стать обязательным.

Кандидат нового Code node и SQL-изменение сессий находятся в `integrations/n8n/`. Они не заменяют архив `legacy/`: перед production-переключением нужно отдельно обновить начальное состояние `/start`, поля Supabase Update node и проверить три ветки диалога. Готовый `presentationRequest` проверяется тем же `userRequestSchema`, что и локальный pipeline.
