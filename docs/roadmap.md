...
- Локальный PptxGenJS рендерер реализует `title`, `sources`, `conclusion`, `definition`, `hero`, `quote`, `two_column`, `three_cards`, `comparison`, `timeline`, `image_text`. Остальные layout из контракта локально отклоняются явно.
...
- Темы презентации расширены до восьми; базовая регрессия и образцы проверены. Последний прогон: 102 теста из 102; `two_column`, `three_cards`, `comparison`, `timeline`, `sources` и `conclusion` просмотрены в реальном экспорте PowerPoint.
