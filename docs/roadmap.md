...
- Отдельный `slidex-renderer-v2` развёрнут в Val Town рядом с legacy. Health вернул 200 и подтвердил bearer/Gemini QC configuration; live POST вернул PPTX правильного MIME с тремя слайдами. ZIP/count, overflow и визуальный QC прошли. URL и token хранятся только в `.env.local`.
- Live deployment выявил две особенности платформы: `vt create` не загрузил вложенные модули, а runtime не использовал `deno.json` как import map. Build теперь записывает pinned `npm:` specifiers, а `npm run valtown:deploy` безопасно синхронизирует все файлы через Files API; по умолчанию команда работает как network-free dry-run.

## Приоритеты

1. Импортировать неактивный `workflow.renderer-v2.json`, подключить отдельные тестовые credentials и выполнить end-to-end прогон через n8n. Старый Val Town URL сохранять для rollback.
2. Расширять визуальный regression-набор реальными презентациями, новыми темами, изображениями и менее очевидными дефектами. После накопления примеров решить, какие AI-решения действительно блокируют выпуск.
3. Прогнать три ветки Telegram-диалога education-context. После этого сделать `educationContext` обязательным и подключить visual policy к Gemini planning. Возраст по группе не угадывать.
4. Собрать канонические презентации на 10 и 13 слайдов из полного набора layouts, проверить визуальный ритм, повторения, источники и точное количество слайдов. Не вводить silent fallback.
5. Добавить отдельную проверку происхождения фактов, статистики, цитат и источников. Визуальная модель не устанавливает истинность содержания.
6. После стабилизации локального рендера сравнить его с рабочим MVP и подготовить переключение n8n с Val Town с возможностью отката.
