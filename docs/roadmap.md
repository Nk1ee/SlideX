...
- Локальный PptxGenJS рендерер реализует `title`, `sources`, `conclusion`, `definition`, `hero`, `quote`, `image_text`. Остальные layout из контракта локально отклоняются явно.
...
- ZIP/PPTX и slide count проверяются программно. Есть ручная Gemini-проверка PNG готового слайда по строгому отчёту; она пока диагностическая, не production-блокер.
- Темы презентации расширены до семи; базовая регрессия и образцы проверены. Последний прогон: 88 тестов из 88 и успешные живые проверки слайдов `sources` и `conclusion`.

## Приоритеты

1. Собрать воспроизводимый пакет визуальной регрессии: canonical JSON, PPTX, PNG и отчёт для каждого проверяемого слайда. Проверять несколько разных layout и намеренно проблемные примеры, чтобы измерить ложные решения AI. После этого решить, какие дефекты действительно блокируют выпуск.
2. По одному переносить оставшиеся layouts из legacy Val Town, начиная с наиболее востребованных. Для каждого — контракт, sample PPTX, ZIP/slide-count check и визуальный просмотр. Не вводить silent fallback.
3. Добавить отдельную проверку происхождения фактов, статистики, цитат и источников. Визуальная модель не устанавливает истинность содержания.
4. После стабилизации локального рендера сравнить его с рабочим MVP и подготовить переключение n8n с Val Town с возможностью отката.
