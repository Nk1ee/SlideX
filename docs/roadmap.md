...
- Локальный PptxGenJS рендерер реализует `title`, `sources`, `conclusion`, `definition`, `hero`, `quote`, `two_column`, `image_text`. Остальные layout из контракта локально отклоняются явно.
...
- Темы презентации расширены до восьми; базовая регрессия и образцы проверены. Последний прогон: 93 теста из 93; `two_column`, `sources` и `conclusion` просмотрены в реальном экспорте PowerPoint.
